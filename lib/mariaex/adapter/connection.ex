  defmodule Mariaex.Adapter.Connection do
    @moduledoc false
    ...
    def prepare_execute(conn, name, sql, params, opts) do
      Mariaex.prepare_execute(conn, name, sql, params, opts)
    end

    @impl true
    def query(conn, sql, params, opts) do
      #opts = Keyword.put_new(opts, :query_type, :binary_then_text)
      Mariaex.query(conn, sql, params, opts)
    end

    @impl true
    def execute(conn, query, params, opts) do
      case Mariaex.execute(conn, query, params, opts) do
    ...
    @impl true
    # def to_constraints(%Mariaex.Error{mysql: %{name: :ER_DUP_ENTRY}, message: message}, opts) do
    #   case :binary.split(message, " for key ") do
    #     [_, quoted] -> [unique: normalize_index_name(quoted, opts[:source])]
    #     _ -> []
    #   end
    # end
    # def to_constraints(%Mariaex.Error{mysql: %{name: name}, message: message}, _opts)
    ...
    defp column_change(_table, {_command, _name, %Reference{validate: false}, _opts}) do
      error!(nil, "validate: false on references is not supported in Mariaex")
    end
    ...
    defp default_expr({:ok, value}) when is_map(value) do
      library = Application.get_env(:mysql, :json_library, Jason)
